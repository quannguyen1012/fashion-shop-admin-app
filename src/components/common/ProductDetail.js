           ...
           style={{width:'40%',height:'20%',
            ...
            <h1 style={{ textAlign: "center", marginTop: 5, color:"rgb(240, 227, 121)"}}>Product detail</h1>
              ...
              <Form.Item label="Color" >
                  {product.productColorDtos && product.productColorDtos.map((color) => (
                      <span key={color.id} value={color.id}>{color.color} &ensp;</span>
                  ))}
              </Form.Item>

              <Form.Item label="Size" >
                  {product.productSizeDtos && product.productSizeDtos.map((size) => (
                      <span key={size.id} value={size.id}>{size.size} &ensp;</span>
                  ))}
              </Form.Item>
              
              <Form.Item label="Category" >
                {product.categoryDto && <span>{product.categoryDto.name}</span>}
              </Form.Item>
